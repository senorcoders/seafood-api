    ...
    checkCartItems: async function (req, res) {
        try {
            ...
            await Promise.all(cart.items.map(async it => {
                //let's check if variation still exists
                let variationExists = await Variations.find( { id: it.variation } );
                console.log( 'exist', variationExists )
                if( variationExists.length == 0 ){
