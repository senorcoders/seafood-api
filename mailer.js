...
async function getTemplateShoppingSeller(sellerName,cart,items,orderNumber) {
    ...
    for (let [index,it] of items.entries()) {
        ...
        price+=subtotal;
        shippingRate+=await require('./api/controllers/ShippingRatesController').getShippingRateByCities( it.fish.city, it.quantity.value )*it.quantity.value;
    }
