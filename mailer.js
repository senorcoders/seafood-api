...
async function getTemplateShoppingSeller(sellerName,cart,items,orderNumber) {
    ...
    for (let [index,it] of items.entries()) {
        ...
        itemsTemplate+=`<div style="display: table;height: 100%;flex-wrap: wrap;box-sizing: border-box;padding: 0 0 2px 5px;align-items: center;width:100%"><div style="width: 30%;float: left;box-sizing: border-box;padding-top: 5px; height: 100%">${img}</div><div style="width: 50%;float: left;box-sizing:border-box;padding: 5px; height: 100%">Shipment 1 of 1 sold by ${it.fish.store.name}<br /><span style="color:blue;font-weight: bold;">${it.fish.name}</span> <br />SKU: ${it.fish.seafood_sku}<br /><span style="color:black;font-weight: bold;">QTY - ${it.quantity.value} ${it.quantity.type}</span></div><div style="width: 20%;float: left;background: #b9b4b4;box-sizing:border-box;padding: 5px;color:black; text-transform: uppercase;font-size: 14px; height: 100%;align-items: center;border-left: 1px solid black;position:relative;display:table"><p style="position: absolute;top: 50%;transform: translate(0,-50%)">${it.fish.price.value* it.quantity.value} AED</p></div></div>`
        let subtotal=it.fish.price.value* it.quantity.value;
        price+=subtotal;
    }
